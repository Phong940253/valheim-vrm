	...
	internal static class RegisterRPCPatch
	{
		...
		private static void Postfix(ZNet __instance)
		{
			...
			foreach (ConfigSync configSync in configSyncs)
			{
				...
				{
					// Already registered from a previous session (ZNet re-instantiated after disconnect) – safe to skip.
					Debug.Log($"[ServerSync] RPC '{configSync.Name} ConfigSync' already registered, skipping duplicate.");
				}
				if (isServer)
				{
